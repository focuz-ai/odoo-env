...
existen en el entorno de desarrollo (`o16-env`):
...
La versión activa se deduce de `odools.toml` o de la rama del repo (`16.0`).
Los estándares específicos de esa versión viven en este `docs/`.

## Resolución de docs

Los agentes globales deben resolver primero `<raíz o16-env>/docs/<tema>.md`.
Mientras tanto, `~/.local/share/odoo-openspec/docs/<tema>.md` actúa como respaldo.

## Validación del entorno

La skill `odoo-doctor` valida que `odools.toml` y el `dev.conf` del cliente estén
listos antes de correr el flujo, sin leer secretos. Ejecútala como paso 0.
