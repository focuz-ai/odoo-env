...
This is an Odoo 20 development environment (o20-env) configured for multi-client/multi-project development with VSCode
...
# Fuera del workspace (convención o20-offsite/ — ver sección Odoo IDE):
../o20-offsite/industry/   # Industry modules (runtime vía addons_path en dev.conf)
```
...
Abre siempre el workspace **`o20-env`** (`o20-env.code-workspace` o la carpeta raíz), **no un subrepo suelto**
...
**Setup (una sola vez, desde la raíz de `o20-env`):**
...
**Settings clave** (`.vscode/settings.json` / `o20-env.code-workspace`):
...
El superproyecto `o20-env` **no** usa `.gitmodules` en la raíz. Los repos en `src/dev/focuz-ai/` son repositorios git
...
**Convención `o20-offsite/`:** los árboles pesados que NO se desarrollan se reubican fuera del workspace, en
`../o20-offsite/`, para no contar contra el límite:

| Reubicado           | Por qué                                                              | Runtime                                                                        |
| ------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `industry/`         | módulos vendored, no se editan                                       | el `addons_path` de los `config/*/dev.conf` apunta a `../o20-offsite/industry` |
